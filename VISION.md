...
The first flow router calls OpenCode Go from the control plane through TanStack AI's OpenAI-compatible adapter. Its API key remains a control-plane secret, and Zen balance fallback stays disabled so routing cannot create usage charges beyond the subscription. A contract benchmark will compare `glm-5.3-flash`, `mimo-v2.5`, and `deepseek-v4-flash` for routing accuracy, clarification behavior, valid structured output, latency, and quota use before Ornn pins one model. Production use requires confirmation that unattended internal routing complies with OpenCode's hosted-service terms.
...
- **Flow router:** uses TanStack AI behind an Ornn-owned interface to select a registered flow or request clarification. OpenCode Go is the first model provider.
...
Agent engines may receive a job-scoped grant for an Ornn-owned web research capability hosted by the trusted Runner. Its implementation may reuse pinned agent tools or an MCP adapter, but it does not grant general network access to the sandbox. Only operator-approved Runner configuration may expose those tools; repository-provided extensions and MCP configuration cannot expand a job's capability grants.
